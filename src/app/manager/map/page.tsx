...
import { Building2, Map, AlertTriangle, Radio, Video, Shield, Navigation, Layers, Plus } from 'lucide-react';
...
function ManagerMapContent() {
    ...
    // Fetch buildings managed by this building manager
    useEffect(() => {
        const fetchBuildings = async () => {
            try {
                const data = await api.getBuildingsWithStatus();
                // Transform API data to display format
                const displayBuildings: BuildingDisplay[] = data.map((b: any) => ({
                    id: b.id,
                    name: b.name,
                    address: b.address,
                    ...
                    society_name: b.society?.name,
                    total_sensors: b.total_sensors || 0,
                    total_cameras: b.total_cameras || 0,
                }));
    ...
    const selectedBuildingData = selectedBuilding ? buildings.find(b => b.id === selectedBuilding) : null;

    if (loading) {
        return (
            <DashboardLayout role={dashboardRole} userName={user?.name || 'Building Manager'} userTitle={roleTitle} disablePadding={true}>
                <div className="h-full flex items-center justify-center min-h-[calc(100vh-64px)]">
                    <div className="flex flex-col items-center gap-3">
                        <div className="animate-spin rounded-full h-10 w-10 border-b-2 border-[#1f3d2f]"></div>
                                ...
                                buildings.map((building) => (
                                    ...
                                    >
                                        ...
                                        {building.society_name && (
                                            <p className="text-xs text-[#1f3d2f] font-medium mt-1">📍 {building.society_name}</p>
                                        )}
                    ...
                    <div className="bg-white/95 dark:bg-card/95 backdrop-blur-md rounded-xl border border-border shadow-lg pointer-events-auto p-3">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-xs font-bold text-foreground">{buildings.reduce((sum, b) => sum + (b.total_sensors || 0), 0)} Online <span className="text-green-500 ml-1">●</span></p>
                            </div>
                        </div>
                    </div>
                </aside>
                    ...
                    <div className="bg-white/95 dark:bg-card/95 backdrop-blur-md rounded-xl border border-border shadow-xl overflow-hidden p-3 pointer-events-auto">
                        ...
                        <div className="space-y-2 text-xs">
                            ...
                            <div className="flex items-center justify-between gap-4">
                                ...
                                <span className="font-bold text-red-600">{activeAlerts || buildings.reduce((sum, b) => sum + b.alerts, 0)}</span>
                            </div>
                        </div>
                    </div>
