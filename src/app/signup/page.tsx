        ...
        <motion.div variants={scaleIn} initial="initial" animate="animate" className="max-w-md w-full">
          <div className="bg-white rounded-2xl shadow-xl p-8">
            <div className="text-center mb-6">
              ...
              <h2 className="text-3xl font-bold text-dark-green-800">{t.signup.title}</h2>
              <p className="text-dark-green-600 mt-2">{t.signup.subtitle}</p>
            </div>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-dark-green-700 mb-2">{t.signup.selectRole}</label>
                <div className="grid grid-cols-3 gap-2">
                  <button type="button" onClick={() => setFormData({...formData, role: 'building_authority'})} className={'p-3 rounded-lg border-2 transition-all border-dark-green-500 bg-dark-green-50'}>
                    ...
                    <span className="text-xs font-medium">{t.signup.management}</span>
                  </button>
                  <button type="button" onClick={() => setFormData({...formData, role: 'resident'})} className={'p-3 rounded-lg border-2 transition-all border-gray-200 hover:border-dark-green-300'}>
                    ...
                    <span className="text-xs font-medium">{t.signup.resident}</span>
                  </button>
                  <button type="button" onClick={() => setFormData({...formData, role: 'firefighter'})} className={'p-3 rounded-lg border-2 transition-all border-gray-200 hover:border-dark-green-300'}>
                    ...
                    <span className="text-xs font-medium">{t.signup.firefighter}</span>
                  </button>
                </div>
              </div>
              <div className="relative">
                <User className="absolute left-3 top-1/2 transform -translate-y-1/2 text-dark-green-400 w-5 h-5" />
                <Input type="text" placeholder={t.signup.fullName} className="pl-11" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} required />
              </div>
              <div className="relative">
                <Mail className="absolute left-3 top-1/2 transform -translate-y-1/2 text-dark-green-400 w-5 h-5" />
                <Input type="email" placeholder={t.signup.email} className="pl-11" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} required />
              </div>
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 transform -translate-y-1/2 text-dark-green-400 w-5 h-5" />
                <Input type={showPassword ? "text" : "password"} placeholder={t.signup.password} className="pl-11 pr-11" value={formData.password} onChange={(e) => setFormData({...formData, password: e.target.value})} required />
              ...
              <Button type="submit" className="w-full" size="lg" disabled={loading}>{loading ? t.signup.creating : t.buttons.createAccount}</Button>
            </form>
            <div className="mt-6 text-center">
              <p className="text-dark-green-600">{t.signup.alreadyHaveAccount} <Link href="/login" className="font-medium text-dark-green-700 hover:text-dark-green-900">{t.signup.signIn}</Link></p>
            </div>
          </div>
        </motion.div>
      </div>
